...
**Status:** Approved by project owner, 31 Aug 2026. Since approval: deployment target revised (§13, Azure → Cloudflare Pages + Render + Neon + R2 + Resend); **M1–M7 built, tested, and live in production** (6 Sep 2026) — including security hardening (§12), a full visual redesign, and a first-run tutorial + `docs/USER_GUIDE.md`. Private GitHub monorepo (§17), CI on every push (lint, tests, migrations, dependency audit).
...
- **Hardening (M7):** login is rate-limited per IP (`slowapi`, default 10/min, `X-Forwarded-For`-aware); new passwords must clear a policy (min length, not common/repetitive — also enforced by `seed_admin`); the API sends `X-Content-Type-Options` / `X-Frame-Options` / `Referrer-Policy` / `Permissions-Policy` (+ HSTS in prod) on every response, and the frontend ships a `_headers` file (built with the API + Sentry origins folded into a CSP `connect-src`); Sentry error tracking (backend + browser), no-op unless a DSN is set; CI runs `pip-audit` + `npm audit` and Dependabot opens weekly PRs. Everything else — E2E/load/accessibility tests, sub-processor DPAs, refresh tokens, SSO, moving off `python-jose` — is in **`docs/PRE_LAUNCH_CHECKLIST.md`**.
...
4. ~~Any branding requirements for the UI or the email templates?~~ **DECIDED (6 Sep 2026): none supplied — the M7 redesign uses a neutral warm palette (off-white / teal) and a plain "PO Tracker" wordmark. Swap in real brand colours/logo via `app/globals.css` tokens + `NavBar.tsx` if the employer provides them.**
...
7. ~~Is a mobile-responsive layout required for v1, or is desktop-only acceptable initially?~~ **DECIDED & BUILT (6 Sep 2026): v1 is responsive.** The M7 redesign adapts every screen — tables scroll horizontally inside their cards, the nav collapses labels to icons, the "Today" strip scrolls, forms go single-column. Not yet done: a formal cross-device accessibility audit (`docs/PRE_LAUNCH_CHECKLIST.md`).
...
- **M8 — Purchase orders, quantities, partial delivery, reports — DONE (8 Sep 2026).** Stakeholder change request; full spec + per-phase notes in **§18**. Promoted "PO" from a text label to a real **Purchase Order** record that owns its lines; added **quantity** and a three-state **delivery status** (Not delivered / Partial / Complete); auto **Delivered** PO when all lines Complete, manual **Closed**/**Cancelled** for managers/admins; **full name** on users, used in reminder emails and every assignee picker ("Name (email)"); reworked the dashboard into clickable cards ("Due in 1–30 days" replaces "Due today"); new **Purchase Orders** list + detail screens; **Reports** screen (`/reports`) anyone can run — overdue / deliveries-in-range / on-time % / by assignee / by status, on-screen + CSV/Excel/PDF. Built in four phases (data model → backend → frontend → reports), each backward-compatible so production kept working between deploys. Alembic `0005` + `0006`. 123 backend tests. **Still open:** the stakeholder never defined the second "Total Lines" dashboard card (only the combined "Purchase orders · PO lines" card was built).
...
- **Output:** on-screen table, **CSV**, **Excel** (`openpyxl`), **PDF** (`fpdf2` — pure Python, no system libs, so the Render build stays simple).
- Backend: `GET /reports` lists them; `GET /reports/{name}?from=&to=&assignee_id=&po=&priority=&delivery_status=&format=json|csv|xlsx|pdf`. `format=json` returns `{columns, rows, summary}`; the file formats come back as an attachment. Every signed-in role can call it (Viewer included); staff still only see their own lines.
- A `po_lines.delivered_at` column (migration `0006`) records when a line last became COMPLETE (cleared on reopen) so "deliveries in a date range" and on-time-% are real, not guessed. Backfilled from `modified_at` for existing COMPLETE rows.
...
2. **Backend — DONE (7 Sep 2026).** `GET/POST /purchase-orders`, `GET /purchase-orders/{id}` (lines nested, staff-visibility 404), `PUT /purchase-orders/{id}` with `{action: close|cancel|reopen}` (manager/admin only; close needs Delivered, reopen recomputes from lines). PO-line create rejects a CLOSED/CANCELLED PO (409); `/po-lines` list gains `delivery_status` / `priority` / `due_within` / `purchase_order_id` filters for the dashboard drill-throughs. Reminder emails now greet by first name and name the assignee ("Alice Adams &lt;alice@…&gt;"). `DashboardSummary` already carried the M8 counts from Phase 1. Still backward-compatible (additive). 107 pytest (10 new across `test_purchase_order_endpoints.py` + a reminder-name test), ruff clean.
3. **Frontend — DONE (8 Sep 2026).** New **Purchase Orders** list (`/purchase-orders`, status filter, "+ New PO") + **PO detail** (`/purchase-orders/detail?id=`, lines table, Close/Cancel/Reopen for managers+admins, "+ Add line" prefilling the PO). **Dashboard** reworked into clickable cards — one combined "Purchase orders / PO lines" card, then POs delivered / POs closed / Due in 1–30 days / Overdue / High priority, each linking to the matching filtered list; greeting uses the user's first name. **PO Lines list** gains Qty + Delivery columns, a drill-through chip, and reads `?due_within=` / `?delivery_status=` / `?priority=` / `?status=` from the URL. **Forms**: quantity field + a Not delivered / Partial / Complete dropdown replace the "Mark as delivered" checkbox; `new` accepts `?po=`. **Users**: full-name field on create, editable name in the table, "Name (email)" in every assignee picker (`lib/status.ts` `assigneeLabel`). Shared `lib/types.ts` + `components/Pill.tsx`. Nav gains "Purchase Orders". typecheck / lint / vitest / build all green.
4. **Reports — DONE (8 Sep 2026).** `app/services/reports.py` — five reports (overdue / deliveries / on-time / by-assignee / by-status) built in Python over the caller's visible lines; `report_formats.py` renders CSV (UTF-8 BOM), XLSX (`openpyxl`), PDF (`fpdf2`). `GET /reports` + `GET /reports/{name}` (`format=` + `from`/`to`/`assignee_id`/`po`/`priority`/`delivery_status` filters), open to every role. Migration `0006` adds `po_lines.delivered_at`, set by `crud.po_line` on the transition to/from COMPLETE. Frontend `/reports` page: report picker, filter form, on-screen table + summary, CSV/Excel/PDF download buttons; "Reports" in the nav. 16 new backend tests (123 total), tsc/lint/vitest/build green.
